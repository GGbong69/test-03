	...
	match page:
		...
		1:
			return [
				...
				#  라운드 경계는 런에 일곱 번뿐이라 고치는 동안 매번 런을
				#  돌 수 없다 — 「쓸기 다시 보기」와 같은 이유다. 위의
				#  「판」 줄로 라운드를 맞추고 이 줄로 넘김을 보면 손가락
				#  두 번이고, 반음 사다리 여덟 단도 그 둘로 다 들어 본다.
				{"n1": "라운드 넘김 다시 보기", "t": "act", "a": "turn"},
		...
		3:
			return [
				...
				#  ⚠ 위 줄은 **토글이다** — 같은 줄을 다시 누르면 대역이
				#  내려가 저장한 대로 돌아온다(「모션 끄기/켜기」와 같은
				#  말투). 아래 줄은 되돌리기가 **아니라** 진짜로 지우는
				#  쪽이다. 슬래시가 그 둘을 갈라 준다. 2026-09-19
...
static func _run(g: Node, e: Dictionary) -> void:
	...
	match String(e.get("a", "")):
		...
		"turn":
			...
			#  건드리고, 연출 함수만 부른다.
			#  **leg_no 를 안 올린다**: 올리면 개발자 모드가 런 진도를
			#  만지는 자리가 된다(_reroll 을 안 부르는 것과 같은 이유다).
			#  라운드는 바로 위 「판」 줄로 옮기고 여기서 본다.
			#  leg_t 를 되감아 카드 딜도 같이 돌린다 — 연출이 딜 **위에**
			#  얹혀 있다는 것이 이 박자의 전부라, 딜 없이 보면 무엇에
			#  얹혔는지가 안 보인다. 2026-09-20
			if g.state != g.S.LEG:
				g._open_leg()
			g.leg_t = 0.0
			g._turn_begin(maxi(GameData.round_of(g.leg_no) - 1, 1))
			_say("라운드 넘김")
			return
		"sweep9":
		...
		#
		#  ⚠ 그 비대칭이 「열기」를 **토글**로 만든다. 대역을 내리는 줄이
		#  「전부 잠그기」 하나뿐이면, 다 찬 화면을 구경하고 되돌리려 그
		#  줄을 누르는 순간 **진짜 진도가 디스크에서 사라진다**(unfound_all
		#  은 바로 flush 한다). 열기는 저장을 안 쓰니 되살릴 길도 없다.
		#  바로 위 「해금」 쌍은 unlock_all 이 표에서 다시 심어 주니 진짜
		#  왕복인데, 발견 쌍만 한쪽이 분쇄기였다. 되돌리기는 **같은 줄을
		#  다시 누르는 것**이고, 「잠그기」는 지우려고 누르는 줄이다.
		#  2026-09-19
		"found_all":
			if g.found_all:
				g._dev_unlock_off()
