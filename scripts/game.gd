...
func _draw_topbar() -> void:
	...
	if state == S.SHOP or state == S.STAGE:
		...
		draw_string(font_sm, Vector2(g.position.x, 13.0), msg,
...
func _panel_slot(i: int) -> void:
	...
	draw_string(font_sm, cell.position + Vector2(0.0, cell.size.y - 3.0),
...
func _cons_draw() -> void:
	...
	draw_string(font_sm, Vector2(box.get_center().x, box.end.y + 9.0),
...
func _card_box(p: Vector2, x: float, w: float, col: Color,
	...
	draw_string(font_sm, p + Vector2(x, 60.0), label,
...
func _leg_card(i: int, rn: int) -> void:
	...
	draw_string(font, Vector2(0.0, 30.0), GameData.leg_name(rn),
			HORIZONTAL_ALIGNMENT_CENTER, sz.x, 11,
...
#  음악 넷도 이제 빌려 온 것이 아니다 — scripts/tools/mus_bake.py 가 굽고
#  그 수치가 저장소 안에 있다. 그래서 이 줄은 여전히 비어 있다.
const CREDITS := ""
...
#  네 파일은 scripts/tools/mus_bake.py 가 **굽는다.** 효과음과 같은 규약
#  이라 원본은 파일이 아니라 그 안의 수치다 — 빌려 온 것이 없으므로
#  음악에도 조건이 안 붙는다(docs/크레딧.md).
const MUS := {
	"lobby":  "res://assets/music/lobby.wav",   # 로비 — 타이틀·설정·컬렉션
	"select": "res://assets/music/select.wav",  # 고르는 자리 — 판 선택·상점·정산
	"game":   "res://assets/music/game.wav",    # 판 위 — 작은 판·큰 판
	"boss":   "res://assets/music/boss.wav",    # 판 위 — 보스 판
}
...
#  겹치는 방식은 네 곡이 **같은 곡의 편곡**일 때 맞다. 빌려 온 넷은 서로
#  다른 곡이고 템포도 85.26~85.68 BPM 으로 흩어져 있어서, 겹치는 1.4초
#  동안 두 곡이 서로 다른 박자로 부딪쳤다. 화음도 서로 남이었다.
#
#  **지금 넷은 한 곡의 네 편곡이다**(mus_bake.py). 그러니 겹쳐도 되는데,
#  겹치지 않는 쪽을 그대로 둔다 — 한 번에 한 곡만 울리는 것이 이 게임의
#  전환(SWAP)과 같은 어법이고, 정말로 겹치고 싶으면 그 자리는 적응형
#  (MUS_ADAPTIVE)이지 크로스페이드가 아니다.
#
...
#
#  **지금은 false 다** — 다만 이유가 바뀌었다.
#
#  그쪽은 네 편곡을 한 타임라인에 묶어 게인만 미는 방식이라, 상점에
#  들어가도 음악이 처음으로 안 돌아간다 — 발라트로가 그렇게 들리는 이유가
#  그것이다. 그 방식은 네 파일이 **같은 곡의 편곡**이어야 성립하는데,
#  빌려 온 넷은 그게 아니었다(템포 0.42 BPM 어긋남 · 길이 0.4초 어긋남 ·
#  셋이 페이드아웃). 그래서 켤 수가 **없었다.**
#
#  지금 넷은 mus_bake.py 가 한 곡의 네 편곡으로 굽는다 — 같은 조·화성·
#  템포(86.00), 같은 길이(1969116 샘플), 같은 마디 자리, Forward 루프.
#  즉 **에셋 쪽 조건은 다 맞았다.** 남은 것은 적응형 관리자 자체를 이
#  게임에 물리고 재는 일이고, 그건 따로 할 일이다(audio/music/README.md).
#  그때까지는 여기 mus_* 가 쥔다.
#
...
#  보스 판의 속도.
#
#  보스는 이미 편곡이 따로다(boss.wav). 그러니 여기서 하는 일은 곡을 한 번
...
func _boost_draw() -> void:
	...
	draw_string(font, Vector2(0.0, c.y + h + 30.0), String(boost_card.get("n", "")),
			HORIZONTAL_ALIGNMENT_CENTER, VIEW.x, 11, Color(C_TXT, 1.0 - tear * 0.7))
