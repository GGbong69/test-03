...
# 못 읽었다. 글자 키우기(2026-09-17)에서 이름을 11 로 올리고 통째로 적는다 —
...
func _pend_name_w(i: int) -> float:
	...
	#  글꼴이 없는 판(헤드리스 프로브)에서는 글자 수로 어림한다 — 한글 11px 이 한 칸 11px.
	if font == null:
...
#  0.5배" 가 크기 11 에서 125px(9 에서 102)이고, 지금 n=3 의 안쪽 폭 141 에
#  든다. n=4(안쪽 99.5)면 11 이 안 들어가 그 카드만 9 로 물러난다(_stage_card).
...
#
#  글자 키우기(2026-09-17)에서 이름 11 → 18 · 설명 9 → 11 로 올렸다. 띠 밑
#  y[10,80] 70px 에 받침(지름 36) · 이름(잉크 17) · 설명(잉크 11)을 다 세우면
#  74px 이라 안 든다 — 이름이 받침 밑동을 물었다. 설명은 **선 카드에만** 있으므로
#  누운 카드는 받침을 그대로 두고 이름만 내리고(_up 없는 값), 서는 만큼 받침이
#  줄며 올라가 설명 자리를 낸다(_up 값). 선 카드는 1.12 배로 커지므로 화면에서
#  받침이 누운 것보다 작아 보이지 않는다.
#    누움  받침 y[10,46] · 이름 잉크 [50,67]
#    섬    받침 y[10,40] · 이름 잉크 [44,61] · 설명 잉크 [65,76] · 그늘 80
const CARD := {
	"x0": 76.0, "y": 140.0, "h": 86.0, "gap": 14.0,
	"icon": 28.0, "icon_r": 17.0, "plate": 18.0, "name": 66.0,
	"icon_up": 25.0, "icon_r_up": 14.0, "plate_up": 15.0, "name_up": 60.0,
	"desc": 75.0,
}
...
#
#  이름은 18(갈무리9 두 배 — 단추 이름과 같은 단), 값은 11. 이름 11 이던 때
#  「UI 에 비해 글자가 작다」 는 말을 들었다(2026-09-17). 창구는 빗변 삼각형이라
#  아래로 갈수록 좁다 — 이름 잉크 y[130,146] 에서 빗변 x 69 · 이름 폭 36,
#  값 줄(베이스라인 165)에서 빗변 59 · 「+12」 가 x[5,39] 에 든다. 값을 22 로
#  올리면 67px 이라 빗변을 넘어 펠트에 올라앉으므로 11 에 둔다.
#  오른쪽 이름은 오른끝 맞춤이다 — 전에는 「구매」 폭(22)을 손으로 빼 x 613 에 섰다.
#  크기(18 · 11)는 그리는 줄에 숫자로 적는다 — qa_ui 가 줄에서 읽는다.
const CHUTE_TXT := {"ly": 18.0, "vdy": 19.0, "x": 5.0}
...
func _chute_label() -> void:
	...
	if photo == "burn" or photo == "clone":
		...
		if bn:
			...
			draw_string(font_sm, Vector2(0.0, ly), lab, HORIZONTAL_ALIGNMENT_RIGHT,
					VIEW.x - ex, 18, C_TXT if live else C_DIM)
		return
	var si: int = hand_i if (hand_st == H.CARRY and hand_src == 1) else sell_sel
	var slive: bool = _can_sell() and si >= 0 and si < owned.size()
	draw_string(font_sm, Vector2(ex, ly), "판매", HORIZONTAL_ALIGNMENT_LEFT, -1, 18,
			C_TXT if slive else C_DIM)
	if slive:
		draw_gold_at(ex, vy, "+%d" % GameData.sell_value(owned[si]), 11, C_GOLD)
	...
	draw_string(font_sm, Vector2(0.0, ly), "구매", HORIZONTAL_ALIGNMENT_RIGHT,
			VIEW.x - ex, 18, (C_TXT if ok else C_MULT) if blive else C_DIM)
	if blive:
		var cs := str(stock[bi].cost)
		draw_gold_at(VIEW.x - ex - gold_w(cs, 11), vy, cs, 11,
...
#  값표 글자는 11(갈무리11 제 크기). 9 였을 때는 값 플라크가 갈무리11 을
#  9 로 줄여 그려 획이 흐렸고, 「UI 에 비해 글자가 작다」 는 말도 들었다
#  (2026-09-17). 높이 13 = 오름 11 + 내림 2.
const BILL := {"sz": 11, "h": 13.0}
...
func _bill_at(i: int, r: Rect2) -> void:
	...
	if bool(s.get("pack", false)):
		...
		draw_string(font, Vector2(r.get_center().x - 40.0, y), pt,
				HORIZONTAL_ALIGNMENT_CENTER, 80.0, 11,
...
#     → 두 판이 겹치려면 |Δu| < 27.6 이고 |Δw| < 13/0.788 = 16.5,
#       즉 면 거리 < √(27.6² + 16.5²) = 32.2 여야 한다(값 11 — 글자 키우기 뒤.
#       9 였을 때는 21.6 · 11.4 · 24.4).
#   · 분리 솔버가 보장하는 최소 면 거리는 동전-다트 32.5 · 보드 확장-다트 34.8 ·
#     동전-동전 44 (GOODS_K 1.16 뒤 — 전에는 28 · 30 · 38).
#     32.2 < 32.5 이므로 겹침이 불가능하다 — 여유가 0.3 뿐이라 값을 더 키우면
#     이 정리가 먼저 무너진다. 배율 전 실측 1500롤 겹침 0, 최소 y차 16.2px.
...
func _bill_one(i: int) -> void:
	...
	var can: bool = not s.sold and gold >= s.cost
	draw_gold_at(it.u - bw * 0.5, _p2g(it.w) + DROP.bill_dy, txt, int(BILL.sz),
...
#  ── 글자 단 ─────────────────────────────────────────
#  제목과 효과(1층)는 18(갈무리9 두 배), 곁줄(봉인 · 골드 · 사용 때 · 못 사는
#  이유 · 목표)과 태그는 11(갈무리11). 전에는 11 · 10 · 9 였다 — 「UI 크기에
#  비해 텍스트가 너무 작다」(2026-09-17)에 단추 이름을 18 로 올린 뒤 툴팁도
#  같은 단으로 맞췄다. 10 은 어느 글꼴의 정수배도 아니라 획이 흐렸다.
#  판 폭은 178 → 240. 제목 18 에서 가장 긴 「더 굿 더 베드 더 트리플」(197)이
#  미니동전 자리(27)와 함께 안쪽 226 에 든다. 본문은 18 에서 한 줄 12자쯤이라
#  긴 효과는 네 줄로 접힌다 — 판이 아래로 자란다(폭은 고정이다).
const TIP := {
	"w": 240.0,          # 판 폭 (고정 — 대상마다 크기가 출렁이면 눈이 다시 초점을 잡는다)
	"pad": 7.0,
	"gap": 6.0,          # 대상과 판 사이
	"title": 22.0,       # 제목 줄 높이 — 18px 제목(잉크 16)에 사이 6
	"title_base": 18.0,  # 판 안쪽 윗변에서 제목 베이스라인까지
	"line": 22.0,        # 18px 본문 줄 높이. 한글은 라틴보다 획이 많아
	                     # 130% 아래면 두 줄이 한 덩어리로 뭉친다
	"base": 17.0,        # 18px 줄의 윗변에서 베이스라인까지
	"line_sm": 16.0,     # 11px 곁줄 높이 — 11px 에 145%
	"base_sm": 12.0,     # 11px 줄의 윗변에서 베이스라인까지 (11px 의 오름)
	"chip_gap": 6.0,     # 본문 아래끝과 태그 줄 사이
	"chip_h": 15.0,      # 태그 한 장의 높이 — 11px 낱말(잉크 10)에 위아래 2~3
	"tag_sz": 11,        # 태그 낱말 크기
...
#  크기에 맞는 글꼴 — 9 · 18 은 갈무리9, 11 · 22 는 갈무리11. 정수배에서만
#  획이 딱 떨어진다(qa_ui ①).
func _tip_font(sz: int) -> Font:
	return font_sm if sz == 9 or sz == 18 else font


func _tip_lh(sz: int) -> float:
	return float(TIP.line) if sz >= 18 else float(TIP.line_sm)


func _tip_base(sz: int) -> float:
	return float(TIP.base) if sz >= 18 else float(TIP.base_sm)


#  툴팁 줄에 붙는 값(골드)의 크기. 플라크 숫자는 갈무리11 이라 11 · 22 만 곧다.
func _tip_gsz(sz: int) -> int:
	return 22 if sz >= 22 else 11
...
# tl  이름 뒤에 11pt 로 이어 붙일 설명 (제약 줄에서만 쓴다)
# sz  18(효과 · 1층) 또는 11(곁줄). 다른 크기는 흐리다(TIP 머리말).
...
func _tip_wrap(t: String, w: float, sz: int) -> PackedStringArray:
	...
	if font == null:
		out.append(t)
		return out
	#  재는 글꼴은 그리는 글꼴이다 — 18 은 갈무리9 로 그리므로 갈무리11 로 재면
	#  줄 끝이 판 밖으로 샌다.
	var f := _tip_font(sz)
...
func _tip_build(hit: Dictionary) -> void:
	...
	match hit.k:
		"rack":
			...
			_tip_add(_tip_eff(it), 18, C_TXT)
			if i == sealed:
				_tip_add("이번 판 봉인", 11, C_MULT)
			if it.get("g", "") != "":
				_tip_add(GameData.gold_text(it.g, it.gv), 11, C_TXT)
		...
		"stock":
			...
			if s.type == "item":
				...
				_tip_add(_tip_eff(s.d), 18, C_TXT)
				if s.d.get("g", "") != "":
					_tip_add(GameData.gold_text(s.d.g, s.d.gv), 11, C_TXT)
			else:
				# 사탕·보드 확장·다트·사진 — 효과 한 줄이면 된다. 분류 해설은 소음이다.
				_tip_add(s.d.d, 18, C_TXT)
				if s.type == "fix":
					_tip_add(GameData.use_at_name(
							String(s.d.get("use_at", "any"))), 11, C_ACC)
		...
		"tag":
			...
			_tip_add(_tag_text(bt), 18, C_ACC)
			_tip_add(_tag_when(bt), 11, C_DIM)
		"pend":
			...
			_tip_add(String(pending_tags[i].get("d", "")), 18, C_ACC)
			_tip_add(String(pending_tags[i].get("w", "")), 11, C_DIM)
		"held":
			if i >= cons.size():
				return
		...
		"stage":
			...
			tip_box = false
			tip_title = sp.d.n
			_tip_add(sp.d.d, 18, C_MULT)
			_tip_add("목표 %d" % sp.target, 11, C_DIM)
		"citem":
			_tip_set_tag("동전")
		...
		"lg":
			...
			if i < ll2.size():
				...
				_tip_add(String(ll2[i].d), 18, C_DIM)
		"cfix":
			_tip_set_tag("사진")
			tip_mark = _col_cell(i % COL_PAGE)
			var fx: Dictionary = GameData.fixtures()[i]
			tip_title = fx.n
			_tip_add(fx.d, 18, C_TXT)
...
# 때문이다. 제목이 18 로 커지며(글자 키우기) 제목 칸이 pad+22 까지 늘었고
# 원반은 제목 잉크 가운데(pad+10)에 앉아 아래끝이 pad+18 이다 — 칸 안에
# 들므로 따로 안 벌린다. 원반과 첫 줄 잉크 사이는 8px 이다.
...
func _tip_body_h() -> float:
	...
	for l in tip_lines:
		...
		# 줄 높이는 크기마다 다르다 — 18 은 22, 11 은 16(_tip_lh).
...
func _tip_draw(sh: Vector2) -> void:
	...
	if not tip_chip.is_empty():
		#  원반은 제목 잉크(pad+2 ~ pad+18)의 가운데에 앉는다.
		draw_item_sticker(p + Vector2(TIP.pad + 8.0, TIP.pad + 10.0), 8.0, tip_chip,
				0.42, 0.0, 0.0, 8)
	...
	#  크기는 효과와 같은 18 이다 — 층은 색이 가른다. 판 안에 안 들 만큼 긴
	#  이름(리그 이름 따위)만 11 로 물러난다 — 자르면 이름이 다른 이름이 된다.
	var tsz := 18
	if font_sm.get_string_size(tip_title, HORIZONTAL_ALIGNMENT_LEFT, -1, 18).x \
			> p.x + sz.x - TIP.pad - tx:
		tsz = 11
	...
	#  18 은 17 · 11 은 12(_tip_base). 18 줄과 11 줄이 섞여도 칸이 안 겹친다.
	...
	for l in tip_lines:
		...
		if l.ic != "":
			_icon_modifier(Vector2(lx + 7.0, y + lh * 0.5), 7.0, l.ic, 0.0, tip_a)
			lx += 18.0
		...
		if l.tl != "":
			...
			draw_string(font, Vector2(lx + tw, bl), l.tl, HORIZONTAL_ALIGNMENT_LEFT,
					p.x + sz.x - TIP.pad - lx - tw, 11, Color(C_DIM, tip_a))
		for wi in l.wr.size():
			#  **접힌 조각마다 _tint 를 부르면 이어감이 줄마다 초기화된다.**
			#  폭이 226px 이라 18 에서 한 줄에 12자쯤, 접힘은 흔한 일이고
	...
	#  칠한 덩어리가 된 뒤라, 태그만 띠를 이고 있었다. 낱말은 11(전에는 9).
	...
	for row in _tip_tag_rows():
		...
		for g in row:
			...
			draw_string(_tip_font(int(TIP.tag_sz)),
					tr.position + Vector2(TIP.tag_pad * 0.5, 12.0), String(g.t),
					HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color(g.c, tip_a))
			tgx += g.w + TIP.tag_gap
...
#  손에 읽힌다.
#
#  글자는 18(갈무리9 두 배) · 곁말 11. 전에는 11 · 9 였다 — 스크림 한 장에
#  글 몇 줄뿐인 화면이라 자리가 넉넉한데 글이 작았다(글자 키우기, 2026-09-17).
#  미리보기 한 줄은 높이 38 → 46 · 간격 46 → 54 — 이름 잉크 [5,21] · 설명 잉크
#  [28,38] 이 든다. 세 줄이 y[74,228] 이다.
	...
	match photo:
		"paint":
			...
			draw_string(font_sm, Vector2(0.0, 28.0), "칠할 칸을 고르세요",
					HORIZONTAL_ALIGNMENT_CENTER, VIEW.x, 18, C_TXT)
		"peek":
			_scrim()
			...
			for i in peek_pick.size():
				...
				draw_string(font_sm, r.position + Vector2(10.0, 21.0),
						String(e.d.get("n", "")), HORIZONTAL_ALIGNMENT_LEFT,
						-1, 18, C_TXT)
				draw_string(font, r.position + Vector2(10.0, 38.0),
						String(e.d.get("d", "")), HORIZONTAL_ALIGNMENT_LEFT,
						r.size.x - 20.0, 11, C_DIM)
				draw_string(font_sm, r.position + Vector2(0.0, 21.0),
						"목표 %d" % int(e.target), HORIZONTAL_ALIGNMENT_RIGHT,
						r.size.x - 10.0, 18, C_GOLD)
			draw_string(font_sm, Vector2(0.0, VIEW.y - 14.0), "아무 데나 눌러 닫는다",
					HORIZONTAL_ALIGNMENT_CENTER, VIEW.x, 18, C_DIM)
...
func _draw_leg() -> void:
	...
	#  9 → 11(글자 키우기, 2026-09-17). 11 의 윗끝 y130 이 덮개(y128) 밑이라
	#  베이스라인은 그대로다. 18 은 펠트 인쇄가 판 이름과 같은 무게가 되어 안 올린다.
	...
	#  부제는 _btn 이 11 로 그린다 — 자르는 자도 11 로 잰다(9 로 재면 긴 효과가
	#  단추 밖으로 샌다).
	if GameData.skippable(leg_no):
		_btn(_leg_skip(), "건너뛴다", _elide(_tag_text(leg_tag),
				_leg_skip().size.x - 16.0, 11), true)
	else:
	...
	#  이름은 11(글자 키우기) — 턱 위 몸(17px)에서 잉크 [3,14] 로 가운데에 앉는다.
	for i in pending_tags.size():
		var r := _pend_rect(i)
		...
		draw_string(font, body.position + Vector2(float(PEND.ico), 14.0),
				...
				HORIZONTAL_ALIGNMENT_LEFT, -1, 11, _ui_ink(key, true))
...
func _leg_card(i: int, rn: int) -> void:
	...
	#  글자는 18(갈무리9 두 배 — 단추 이름과 같은 단). 11 · 9 였을 때 「UI 에 비해
	#  글자가 작다」 는 말을 들었다(2026-09-17). 카드 제 좌표(높이 86)에서
	#    윗단 띠 끝 9 · 이름 잉크 [19,36] · 목표 잉크 [43,60] · 금화 [70,75] · 그늘 80
	#  (잉크는 _sign_text 의 한 칸 자국까지). 가장 긴 「목표 20000」 이 95px < 카드 153.
	#  깨진 판의 몸통 폭 80 에도 「작은 판」(61)이 든다 — 금은 x 89 보다 오른쪽이다.
	...
	if broken:
		...
		_sign_text(Vector2(cx - 40.0, 35.0), 80.0, GameData.leg_name(rn), 18,
				Color(paint.darkened(0.35), 0.9), font_sm)
		_sign_text(Vector2(cx - 40.0, 59.0), 80.0, "넘김", 18,
	...
	_sign_text(Vector2(0.0, 35.0), w, GameData.leg_name(rn), 18, ink, font_sm)
	if skipped:
		_sign_text(Vector2(0.0, 59.0), w, "건너뜀", 18, Color(paint.darkened(0.5), 0.85),
				font_sm)
	else:
		_sign_text(Vector2(0.0, 59.0), w, "목표 %d" % GameData.target_of(rn), 18, ink,
				font_sm)
		...
		for ci in mini(rw, 8):
			draw_plaque(Vector2(cx0 + float(ci) * cw, 70.0), 5.5, 3.6, C_GOLD)
	draw_set_transform(shake_off)
...
func _draw_stage() -> void:
	...
	#  9 → 11(글자 키우기) — _draw_leg 의 「라운드」 줄과 같은 단 · 같은 자리다.
	draw_string(font, Vector2(0.0, TBL.fy + 13.0),
...
# 안 섞인다. 이름은 11(글자 키우기 — 전에는 9). 최장 「천체 고리」 49px < 칸 52px.
...
func _apron_mods() -> void:
	...
	for i in n:
		...
		draw_string(font, Vector2(mx - step * 0.5, _apron_y() + 30.0),
				_elide(String(GameData.mod_of(mid).get("n", "")), step - 2.0, 11),
				HORIZONTAL_ALIGNMENT_CENTER, step, 11, C_OFF)
...
func _stage_card(i: int) -> void:
	...
	#  이름 18 · 설명 11(글자 키우기, 2026-09-17 — 전에는 11 · 9). 자리는 CARD 가 쥔다.
	_sign_text(Vector2(0.0, roundf(lerpf(float(CARD.name), float(CARD.name_up), up))),
			sz.x, md.n, 18, ink, font_sm)
	# 효과는 일어서야 보인다. 누운 카드에 여덟 글자를 눕혀 두면 못 읽는다.
	#  카드가 좁아 11 이 안 들면(stage_picks 4 이상) 그 줄만 9 로 물러난다.
	if up > 0.02:
		var dw: float = sz.x - 12.0
		if font.get_string_size(String(md.d), HORIZONTAL_ALIGNMENT_LEFT, -1, 11).x <= dw:
			_sign_text(Vector2(6.0, CARD.desc), dw, md.d, 11,
					Color(ink.darkened(0.12), up))
		else:
			_sign_text(Vector2(6.0, CARD.desc), dw, md.d, 9,
...
func _draw_shop() -> void:
	...
	if pay_msg_t > 0.0:
		...
		ma = 0.85
	#  9 → 11(글자 키우기, 2026-09-17). 리롤(끝 184)과 다음 판(432) 사이
	#  x[188,428] 의 가운데에 선다 — 화면 가운데(320)에 두면 오른쪽 여유가 112 뿐이라
	#  긴 문구가 「다음 판」 에 닿는다. 가장 긴 「It's Not About Money — 동전을
	#  고른다」 가 11 에서 214px 로 든다. 18 은 이 자리에 안 든다. 넘치면 뒤를 자른다.
	if msg != "":
		draw_string(font, Vector2(188.0, 316.0), _elide(msg, 240.0, 11),
				HORIZONTAL_ALIGNMENT_CENTER, 240.0, 11, Color(C_MULT, ma))


func _draw_over() -> void:
	_scrim()
	var e: float = _ease_enter(over_t / maxf(_mo("panel"), 0.001)) 			if _mo("panel") > 0.0 else 1.0
	#  판 높이 218 → 262. 글자를 키우며(2026-09-17 「UI 에 비해 글자가 작다」)
	#  수 줄이 16 → 22 간격이 되어, 실패 화면의 넷째 줄(마지막 판)이 해금 줄과
	#  겹칠 자리였다. 화면 세로 가운데(49~311)에 선다.
	var p := Rect2(74.0, 49.0, VIEW.x - 148.0, 262.0)
	_panel(p, true, e)
	...
	#  곁말 9 → 18(갈무리9 두 배). 결과(33) 밑 한 줄이라 자리가 넉넉하다.
	draw_string(font_sm, Vector2(x0, p.position.y + 72.0),
			"라운드 %d · %d판째" % [GameData.round_of(leg_no), leg_no],
			HORIZONTAL_ALIGNMENT_LEFT, -1, 18, Color(C_DIM, e))

	#  이번 런이 어땠나. 왼쪽은 수, 오른쪽은 마지막까지 든 것.
	#  이름 9 → 11(갈무리11) · 수 11 → 18(갈무리9 두 배). 가장 긴 줄 「마지막 판 ·
	#  99999 / 99999」 가 이름 50 + 수 129px 라 칸 폭(210)에 든다.
	var cw: float = (p.size.x - 56.0) * 0.5
	var xr: float = x0 + cw + 16.0
	var y: float = p.position.y + 104.0
	var rows := [
	...
	for i in rows.size():
		draw_string(font, Vector2(x0, y + float(i) * 22.0),
				String(rows[i][0]), HORIZONTAL_ALIGNMENT_LEFT, -1, 11,
				Color(C_DIM, e))
		draw_string(font_sm, Vector2(x0, y + float(i) * 22.0),
				String(rows[i][1]), HORIZONTAL_ALIGNMENT_RIGHT, cw - 8.0, 18,
				Color(C_TXT, e))

	#  오른쪽도 같은 단이다 — 이름 11 · 값(없음) 18 · 다트통 이름 11.
	draw_string(font, Vector2(xr, y), "마지막까지 든 것",
			HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color(C_DIM, e))
	if owned.is_empty():
		draw_string(font_sm, Vector2(xr, y + 28.0), "없음",
				HORIZONTAL_ALIGNMENT_LEFT, -1, 18, Color(C_OFF, e))
	else:
	...
	#  쪽지 글자 9 → 11(갈무리11), 쪽지 높이 14 → 18. 판 윗변에서 재어 수 넷째 줄
	#  (기준선 170) 아래 180 에 선을 긋고 그 밑(192~210)에 앉는다 — 「새 런」
	#  줄(222~)과 12px 떨어진다.
	if not run_unlocked.is_empty():
		...
		for i in run_unlocked.size():
			...
			var tw: float = font.get_string_size(t, HORIZONTAL_ALIGNMENT_LEFT,
					-1, 11).x + 12.0 if font != null else 60.0
			_rr(self, Rect2(ux, uy, tw, 18.0), Color(C_PANEL.darkened(0.3), ua))
			_rr_left(self, Rect2(ux, uy, tw, 18.0), Color(C_GOLD, ua))
			draw_string(font, Vector2(ux + 6.0, uy + 13.0), t,
					HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color(C_GOLD, ua))
			ux += tw + 8.0

	#  「새 런」 줄 18 → 26px — 이름이 18 이 되어 잉크 16 이 칸 한가운데 선다.
...
#  줄 높이 22 → 24. 글자를 11 에서 18(갈무리9 두 배)로 키우며(2026-09-17 「UI 에
#  비해 글자가 작다」) 잉크 16px 이 22px 띠에 위아래 3px 만 남기고 꽉 찼다.
#  넷이 y 190~298 이고 프로필 패(318~)와 20px 떨어진다.
const TMENU := {"x": SAFE, "y": 190.0, "h": 24.0, "gap": 4.0, "w": 148.0}
...
#
#  줄 높이 34 → 42. 이름 11 → 18 · 밑줄 9 → 11 로 키웠다(2026-09-17 「UI 에 비해
#  글자가 작다」). 이름 띠 22px(잉크 16) + 밑줄 한 줄이 든다. 셋이 y 96~238 이다.
# ══════════════════════════════════════════════════════════
const PROW := {"x": 16.0, "y": 96.0, "w": 176.0, "h": 42.0, "gap": 8.0}
...
func _prof_del_rect() -> Rect2:
	return Rect2(Vector2(232.0, 266.0), Vector2(140.0, 32.0))
...
func _draw_profile() -> void:
	...
	for i in Save.SLOTS:
		...
		#  이름 18(갈무리9 두 배) · 밑줄 11(갈무리11). 11 · 9 였을 때 「UI 에 비해
		#  글자가 작다」 는 말을 들었다(2026-09-17). 띠 22px 에 잉크 16 이 3px 씩 남기고
		#  서고, 밑줄은 띠 아래 한 칸을 띄워 앉는다(기준선 36 · 칸 높이 42).
		_row_band(self, Rect2(r.position, Vector2(r.size.x, 22.0)), ee, ew, 1.0)
		draw_string(font_sm, r.position + Vector2(0.0, 19.0), "프로필 %d" % sl,
		...
		draw_string(font, r.position + Vector2(0.0, 36.0), line,
				HORIZONTAL_ALIGNMENT_LEFT, -1, 11, C_OFF.lerp(C_DIM, ee))
	...
	#  오른쪽 — 고른 줄의 속. 머리와 줄을 18(갈무리9 두 배)로 — 11 일 때 판 폭 300 에
	#  가장 긴 줄(「한 판 최고 점수 · 99999」)이 절반도 안 찼다. 머리 기준선은 왼쪽
	#  첫 줄 이름(96 + 19)과 같은 높이라 두 쪽이 한 줄에서 시작한다.
	var sel := clampi(prof_sel, 1, Save.SLOTS)
	var inf2 := Save.slot_info(sel)
	var px := 232.0
	draw_string(font_sm, Vector2(px, 115.0), "프로필 %d" % sel,
			HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_TXT)
	draw_rect(Rect2(px, 122.0, 300.0, 1.0), Color(C_WIRE, 0.35))
	if not bool(inf2.get("used", false)):
		draw_string(font_sm, Vector2(px, 148.0), "아직 아무것도 없다",
				HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_DIM)
		draw_string(font, Vector2(px, 170.0), "고르면 여기서부터 쌓인다",
				HORIZONTAL_ALIGNMENT_LEFT, -1, 11, C_OFF)
	else:
		var rows := [
		...
		#  줄 간격 17 → 24 — 잉크 16 에 8 을 띄운다. 다섯 줄이 148~246 이고
		#  지우기(266~)와 20px 떨어진다.
		for k in rows.size():
			var y := 148.0 + float(k) * 24.0
			draw_string(font_sm, Vector2(px, y), String(rows[k][0]),
					HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_DIM)
			draw_string(font_sm, Vector2(px, y), String(rows[k][1]),
					HORIZONTAL_ALIGNMENT_RIGHT, 300.0, 18, C_TXT)
		#  지우기 — 겨눈 동안에만 붉다. 몸은 _prof_del_face 가 칠한다(칠한 덩어리 +
		#  턱). 이름은 18 로 몸 한가운데에 선다 — 몸 29px 에 잉크 16.
		var dr := _prof_del_rect()
		var armed: bool = prof_arm == sel
		var db := _prof_del_face(dr, armed)
		draw_string(font_sm, db.position + Vector2(0.0, roundf(db.size.y * 0.5 + 8.0)),
				"정말 지운다" if armed else "지우기",
				HORIZONTAL_ALIGNMENT_CENTER, db.size.x, 18, C_TXT)
...
func _prof_badge_draw() -> void:
	...
	var cur := Save.slot()
	#  이름 11 → 18(갈무리9 두 배). 몸 27px 에 잉크 16 이 한가운데 선다.
	#  68px 이라 x 25~93 이고 자리 점(110~)과 17px 떨어진다.
	draw_string(font_sm, Vector2(b.position.x + 25.0,
			b.position.y + roundf(b.size.y * 0.5 + 8.0)),
			"프로필 %d" % cur, HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_TXT)
...
func _draw_title() -> void:
	...
	for i in TITLE_ROWS.size():
		...
		#  글줄 11 → 18(갈무리9 두 배) — 판 위 단추 이름과 같은 크기다. 줄 24px 에
		#  잉크 16 이 기준선 20 으로 한가운데 선다.
		var br := r
		if font_sm != null:
		...
		draw_string(font_sm, r.position + Vector2(0.0, 20.0),
				String(TITLE_ROWS[i].n), HORIZONTAL_ALIGNMENT_LEFT, -1, 18,
				C_DIM.lerp(C_TXT, ee))
	_prof_badge_draw()
	# 빌려 온 것을 적는 자리. 빌린 것이 있으면 그 라이선스가 이 줄을
	# **조건으로** 단다 — 그때는 지우면 못 낸다. 비어 있으면 안 그린다.
	# 마지막 단추가 342 에서 끝나므로 그 아래 남는 18px 에 앉힌다.
	# 9 → 11(갈무리11) — 다른 곁말과 같은 단이다.
	if CREDITS != "":
...
#  6px 내렸다 — 줄 사각(293~307)과 안 닿는다. 둘 다 화면 바닥(352) 안이다.
func _newrun_go() -> Rect2:
	return Rect2(Vector2(236.0, 312.0), Vector2(168.0, 34.0))


func _newrun_back() -> Rect2:
	return Rect2(Vector2(44.0, 314.0), Vector2(88.0, 30.0))
...
func _draw_newrun() -> void:
	...
	#  이름 11 → 22(갈무리11 두 배). 판의 머리라 한 단을 건너 올린다 — 18 은
	#  갈무리9 라 깨진 글자(_gl_string, 갈무리11)와 글꼴이 갈린다. 가장 긴
	#  「넓은 동전 슬롯」 이 150px 라 글 칸(230~540)에 들고, 밑획(88)과 안 닿는다.
	if hid:
		#  통만 깨지고 글자는 말짱하면 「그림이 고장났다」로 읽힌다.
		#  깨진 것은 신호고, 신호에는 글자도 들어 있다.
		_gl_string(Vector2(230, 83), nm, 22, C_DIM)
	else:
		draw_string(font, Vector2(230, 83), nm,
				HORIZONTAL_ALIGNMENT_LEFT, -1, 22, C_TXT if open else C_DIM)
	...
	#  리그 이름 11 → 18(갈무리9 두 배). 칩 밑의 완주 금줄(221~223)과 고른 칩의
	#  테(~222)에서 4px 띄워 잉크가 227~243 에 선다.
	draw_string(font_sm, Vector2(0, 243), String(cur.get("name", "")),
			HORIZONTAL_ALIGNMENT_CENTER, VIEW.x, 18, lc)
	#  여기도 상자 대신 획 하나다.
	...
	for li in sl.size():
		...
		draw_string(font, _league_line_at(li),
				String(sl[li].n), HORIZONTAL_ALIGNMENT_LEFT, 150.0, 11, C_TXT)
...
func _league_line_rect(li: int) -> Rect2:
	...
	return Rect2(p.x - 2.0, p.y - 11.0, 150.0, 14.0)
...
const SET := {
	...
	"y": 106.0,        # 첫 줄
	#  줄 높이 22 → 24 — 글줄을 18(갈무리9 두 배)로 키우며 잉크 16 이 22px 띠에
	#  꽉 찼다. 제목 메뉴(TMENU)와 같은 높이다. 여섯 줄이 106~284 에 든다.
	"h": 24.0,         # 줄 높이
	"gap": 4.0,
...
#  sz — 글자 크기. 11 이면 갈무리11 을 몸 높이의 0.74 에, 18 이면 갈무리9 두 배를
#  몸 한가운데(잉크 16)에 세운다. 부르는 쪽이 고른다: 런 정보 탭(두 글자)은 18 이
#  들고, 컬렉션 탭은 「보드 확장 12」 가 18 에서 104px 라 95px 칸을 넘어 11 이다.
func _tab_draw(c: CanvasItem, r: Rect2, label: String, on: bool,
		hot := false, sz := 11) -> void:
	...
	_rr(c, body, bc)
	var big: bool = sz == 18
	var ly: float = roundf(body.size.y * 0.5 + 8.0) if big else body.size.y * 0.74
...
func _back_row(c: CanvasItem, r: Rect2, label: String, key: String,
	...
	#  이름 11 → 18(갈무리9 두 배) · 곁말 9 → 11(갈무리11). 판 위 단추 이름과
	#  같은 크기다(2026-09-17 「UI 에 비해 글자가 작다」). 잉크 16 을 칸 한가운데에
	#  세우므로 **칸이 24px 이상**이어야 띠 밖으로 안 샌다 — 부르는 자리(새 런 ·
	#  프로필 · 컬렉션 · 런 정보 · 런 끝)가 다 26px 이상이다.
	var lw: float = font_sm.get_string_size(label, HORIZONTAL_ALIGNMENT_LEFT,
			-1, 18).x if font_sm != null else 0.0
	var kw: float = font.get_string_size("   " + key, HORIZONTAL_ALIGNMENT_LEFT,
			-1, 11).x if font != null and key != "" else 0.0
	var x0: float = r.position.x + (r.size.x - lw - kw) * 0.5
	var y: float = r.position.y + roundf(r.size.y * 0.5 + 8.0)
	c.draw_string(font_sm, Vector2(x0, y), label, HORIZONTAL_ALIGNMENT_LEFT,
			-1, 18, C_DIM.lerp(C_TXT, e))
	if key != "":
...
func _draw_settings(c: CanvasItem) -> void:
	...
	for i in rows.size():
		...
		#  글줄 11 → 18(갈무리9 두 배) · 게이지 수 9 → 11(갈무리11). 「UI 에 비해
		#  글자가 작다」(2026-09-17). 가장 긴 「로비로 나가기」가 115px 라 148 폭에
		#  들고, 수(100 = 20px)와도 안 닿는다(「효과음」 54px).
		c.draw_string(font_sm, r.position + Vector2(0.0, 20.0),
		...
		if bool(info.get("g", false)):
			...
			c.draw_string(font, r.position + Vector2(0.0, 18.0),
					"%d" % int(round(vv * 100.0)),
					HORIZONTAL_ALIGNMENT_RIGHT, r.size.x - 6.0, 11,
...
func _set_panel(c: CanvasItem, key: String, e: float) -> void:
	...
	#  한 줄 설명 9 → 11(갈무리11). 가장 긴 「이 런을 버리고 제목 화면으로
	#  돌아간다」 가 201px 라 판 안 폭(346)에 든다.
	c.draw_string(font, p.position + Vector2(20.0, 72.0),
			String(info.get("d", "")), HORIZONTAL_ALIGNMENT_LEFT, -1, 11,
			Color(C_DIM, pe))

	#  지금 값 — 전체화면의 켬/끔
	if info.has("v"):
		c.draw_string(font, p.position + Vector2(20.0, 118.0),
				String(info["v"]), HORIZONTAL_ALIGNMENT_LEFT, -1, 22,
				Color(C_ACC, pe))
	#  게이지
	if bool(info.get("g", false)):
		...
		#  수 11 → 18(갈무리9 두 배). 「100」 이 29px 라 오른쪽 44px 칸에 든다.
		#  홈(7px)의 한가운데(+4)에 잉크 16 의 한가운데(기준선 −8)를 맞춘다.
		c.draw_string(font_sm, Vector2(p.end.x - 20.0 - 44.0, tr.position.y + 12.0),
				"%d" % int(round(v * 100.0)), HORIZONTAL_ALIGNMENT_RIGHT,
				44.0, 18, Color(vcol, pe))
...
#
#  글자는 18(갈무리9 두 배)이다. 구획 머리가 11 · 줄이 9 였을 때 「라운드 1 / 8」
#  같은 줄이 빈 판 한가운데 작게 떠 있었다 — 「UI 에 비해 글자가 작다」
#  (사용자, 2026-09-17). 트랙 표의 열 이름만 11 이다 — 열 이름은 그 밑의 줄과
#  갈려야 한다.
const RI := {
	"tab_h": 24.0,     # 탭 높이(턱 2 포함). 18 글자의 잉크 16 이 몸 22 에 든다
	"top": 42.0,       # 판 윗변 → 첫 구획. 탭(8~32) 아래 10px
	"foot": 40.0,      # 판 아랫변 위로 비워 둘 몫 — 「뒤로」 줄 26 + 아래 8 + 틈 6
	"head": 32.0,      # 구획 머리(18) + 밑줄 → 첫 줄 자리
	"row": 22.0,       # 18 글줄 간격. 줄 기준선은 제 자리 + 10
	"photo": 50.0,     # 사진 한 장 — 이름(18) + 효과(18) 두 줄
	"coins": 36.0,     # 동전 머리 줄 — 머리 오른쪽에 그림이 같이 선다
	"coin_x": 120.0,   # 그 줄에서 그림이 시작하는 자리(「동전 5 / 5」 82px 오른쪽)
	"trk": 28.0,       # 트랙 표 — 열 이름(11) + 밑줄 → 첫 줄 자리
}
...
#  「뒤로」 줄. 26px — 18 글자가 칸 한가운데 서고(_back_row), 판의 턱(PANEL_LIP)
#  위로 5px 뜬다.
...
func _draw_runinfo() -> void:
	...
	for t in RI_TABS.size():
		_tab_draw(self, _ri_tab_rect(t), RI_TABS[t], t == runinfo_tab,
				_ri_tab_rect(t).has_point(mouse_at), 18)
...
# 머리 · 줄 다 18(갈무리9 두 배). 높이는 _ri_block_h 와 같은 자다 — 머리 32 + 줄마다 22.
func _ri_block(x: float, y: float, w: float, head: String, rows: Array) -> float:
	draw_string(font_sm, Vector2(x, y + 16.0), head, HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_ACC)
	draw_rect(Rect2(x, y + 20.0, w, 1.0), Color(C_WIRE.darkened(0.3), 0.5))
	y += float(RI.head)
	for ln in rows:
		draw_string(font_sm, Vector2(x + 4.0, y + 10.0), String(ln),
				HORIZONTAL_ALIGNMENT_LEFT, w - 8.0, 18, C_TXT)
		y += float(RI.row)
	return y
...
# 레벨 칸을 비운다.
#
# 줄은 18(갈무리9 두 배) · 열 이름은 11(갈무리11). 열 자리는 18 의 가장 긴 값에
# 맞춰 다시 쟀다 — 트랙 이름 「아우터 불」 79px · 점수 「판 숫자」 61px ·
# 배수 「x3」 · 맞힘 세 자리 29px. 판 안 폭 460 에서 왼쪽 두 열과 오른쪽 세 열 사이가 넉넉하다.
...
func _ri_tracks(p: Rect2) -> void:
	...
	for a in GameData.areas_all():
		...
		draw_string(font_sm, Vector2(x + float(RI_TRK.lv), by),
				...
				-1, 18, C_GREEN.lightened(0.2) if lv > 0 else C_DIM)
		draw_string(font_sm, Vector2(x + float(RI_TRK.name), by), String(a.get("n", "?")),
				HORIZONTAL_ALIGNMENT_LEFT, xs - x - float(RI_TRK.name) - 8.0, 18, C_TXT)
		...
		draw_string(font_sm, Vector2(xs, by), sct,
				HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_CHIP)
		draw_string(font_sm, Vector2(xm, by), "x%d" % ml,
				HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_MULT)
...
func _ri_photos(p: Rect2) -> void:
	...
	var y: float = _ri_block(x, p.position.y + float(RI.top), w,
			"지금 든 사진 %d" % own.size(), [])
	if own.is_empty():
		draw_string(font_sm, Vector2(x + 4.0, y + 10.0), "아직 없다",
				HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_DIM)
		return
	for f in own:
		draw_string(font_sm, Vector2(x + 4.0, y + 10.0), String(f.n),
				HORIZONTAL_ALIGNMENT_LEFT, w - 8.0, 18, C_TXT)
		draw_string(font_sm, Vector2(x + 4.0, y + 32.0), String(f.d),
				HORIZONTAL_ALIGNMENT_LEFT, w - 8.0, 18, C_DIM)
		y += float(RI.photo)
...
func _ri_carry(p: Rect2) -> void:
	...
	#  있던 자리다. 그림(반지름 12)의 한가운데를 머리 잉크의 한가운데(기준선 −8)에 맞춘다.
	var cxw: float = xl + float(RI.coin_x)
	draw_string(font_sm, Vector2(xl, y0 + 18.0), "동전 %d / %d"
	...
	if owned.is_empty():
		draw_string(font_sm, Vector2(cxw, y0 + 18.0), "없음",
				HORIZONTAL_ALIGNMENT_LEFT, -1, 18, C_OFF)
	else:
		var step: float = minf(34.0, (xl + p.size.x - 32.0 - cxw - 24.0)
				/ float(owned.size()))
		for i in owned.size():
			draw_item_sticker(Vector2(cxw + 12.0 + float(i) * step, y0 + 10.0), 12.0,
					owned[i], 0.0, 0.0, 0.55 if i == sealed else 0.0, 9)
	y0 += float(RI.coins)
...
func _draw_collect() -> void:
	...
	for i in _col_count():
		...
		match collect_tab:
			...
			5:
				...
				nm = mo.n
		#  이름 9 → 11(갈무리11). 칸(86px)에 안 드는 긴 이름만 한 단 내린다 —
		#  갈무리9 의 9(「LIVE. DIE. REPEAT.」 75px), 그래도 안 들면 예전처럼
		#  _draw_fit 이 줄여 넣는다(「It's Not About Money」 93px). 자르면 뜻이
		...
		if font != null and font.get_string_size(nm, HORIZONTAL_ALIGNMENT_LEFT,
				-1, 11).x <= nw:
			draw_string(font, Vector2(cell.position.x, cell.end.y - 7.0), nm,
					HORIZONTAL_ALIGNMENT_CENTER, cell.size.x, 11, ncol)
		elif font_sm != null and font_sm.get_string_size(nm,
				HORIZONTAL_ALIGNMENT_LEFT, -1, 9).x <= nw:
			draw_string(font_sm, Vector2(cell.position.x, cell.end.y - 8.0), nm,
					HORIZONTAL_ALIGNMENT_CENTER, cell.size.x, 9, ncol)
		else:
	...
	if _col_pages() > 1:
		...
		#  쪽 번호 9 → 11(갈무리11). 격자 끝(304)과 화살표 · 뒤로(322~) 사이다.
...
const TUTOR := {
	...
	#  말상자 글은 18(갈무리9 두 배) · 발치(걸음 · 계속 · 건너뛰기)는 11.
	#  전에는 11 · 9 에 폭 300 이었다 — 「UI 에 비해 글자가 작다」(2026-09-17).
	#  폭을 440 으로 넓혀 가장 긴 문구(u_rack 둘째, 18 에서 420px)가 한 줄에 든다.
	"box_w": 440.0,      # 말상자 폭. 18 에서 23자가 한 줄로 든다
	"box_pad": 8.0,
	"line": 22.0,        # 글 한 줄 높이 — 18px 잉크 16 + 사이 6
	"foot": 28.0,        # 발치 줄 — 건너뛰기 단추(20)와 위아래 4
	"skip": Vector2(72.0, 20.0),   # 건너뛰기 단추(턱 4 포함). 11px 낱말 44 + 여백
}
...
func _tutor_draw() -> void:
	...
	if n > 1:
		draw_string(font, Vector2(bx + bp, fy),
				"%d / %d" % [tutor_i + 1, n], HORIZONTAL_ALIGNMENT_LEFT, -1, 11,
				Color(C_DIM, a))
	...
	draw_string(font, Vector2(sb.position.x, sb.position.y + 13.0), "건너뛰기",
			HORIZONTAL_ALIGNMENT_CENTER, sb.size.x, 11,
			Color(_ui_ink("tutor:skip", true), a))
	draw_string(font, Vector2(bx, fy), "눌러서 계속",
			HORIZONTAL_ALIGNMENT_RIGHT, sk.position.x - bx - 8.0, 11, Color(C_DIM, a))
...
func _tutor_wrap(tx: String, w: float) -> PackedStringArray:
	...
	for word in tx.split(" ", false):
		...
		#  18px(갈무리9 두 배)에서 한 글자 18px 이라 그 값을 쓴다 — 재는 자리가
		#  없다고 검사가 **조용히 통과해 버리는** 것을 막는 쪽이 낫다.
		#  재는 글꼴 · 크기는 그리는 것(_tutor_draw 의 18)과 같아야 한다.
		var tw: float = float(try_s.length()) * 18.0
		if font_sm != null:
