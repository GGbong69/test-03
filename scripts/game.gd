...
const SWAP := {
	...
	# 판이 뜨기 시작하는 시각. 테이블은 t=0.084 에 이미 화면(640) 밖이고
	# 판 윗변이 360 을 넘는 것은 그 2ms 뒤다 — 경계에 딱 붙였다. 더 늦추면
	# 벽만 있는 프레임이 늘고, 당기면 두 층이 겹친다.
	"lead":  0.09,
	"rise":  0.20,    # 판이 서기까지 (lead 부터)
	# 돌아올 때 판이 눕기까지 (0 부터). 서는 것보다 빨라야 한다 — lead 에
	# 테이블이 들어오기 시작하므로 그전에 판이 화면 밖으로 내려가 있어야
	# 두 층이 안 겹친다. 0.15 로는 4px 늦었다.
	"fall":  0.12,
	"tin":   0.20,    # 돌아올 때 테이블이 드는 시간 (lead 부터)
...
func _draw() -> void:
	...
	if swap_live:
		# 뒷벽을 통일한다. 테이블 화면의 바탕은 벽(C_WOOD)이고 판 화면의
		# 바탕은 C_BG 다. 그냥 밀면 두 색의 경계선이 화면을 쓸고 지나가
		# "테이블이 빠진다" 가 아니라 "색이 갈린다" 로 읽힌다. 벽은 안 밀고
		# 제자리에서 판 화면의 바탕색으로 **바뀐다** — 카운터만 옆으로
		# 빠지고 방은 그대로 있는 것이 실제로 일어나는 일이다.
		draw_rect(Rect2(Vector2.ZERO, VIEW), C_WOOD.lerp(C_BG, _swap_rise()))
