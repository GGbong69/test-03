...
# 상점 — 진짜 3D 로 다시 짠다.
#
# 옛 상점은 손으로 계산한 정사영이었다. 테이블은 52° 로 내려다보고 상인은
...
func _ready() -> void:
	# glb 인스턴스는 임포터 재질을 쓴다. 도트 셰이더로 갈아 끼운다.
	for who in ["Table", "Cap", "Dart", "Dealer"]:
		_skin(who)
	_aim()


# 상인은 glb 인스턴스라 임포터가 붙인 재질을 쓴다. 도트 셰이더로 갈아
...
# ── 카메라 ───────────────────────────────────────────
# 옛 2D 상점의 구도다 — 테이블이 화면을 좌우로 넘치고 상인은 상반신만
