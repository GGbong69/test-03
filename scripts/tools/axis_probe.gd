...
#  잰다. 판이 넘어갈 때마다 동전 슬롯·트랙·보드 확장·자루·사진을 통째로 찍어 두고,
#  판마다 실제로 난 점수와 같이 낸다.
#
#  내는 것
#    · 판별 평균: 목표 · 실제 낸 점수 · 쓴 다트 · 여유배율
#    · 판별 평균 보유: 동전 수 · 트랙 레벨 합 · 보드 확장 수 · 사진 수 · 비표준 자루
#    · 각 축이 상한(동전 5칸 · 트랙 40레벨 · 보드 확장 8 · 사진 7)에 닿는 판
...
func _finish() -> void:
	print("\n판  A  종류      목표    통과율  평균다트  여유   낸점수   동전 트랙 보드 확장 사진 자루  x곱")
