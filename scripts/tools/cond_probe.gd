...
#    ② 실전 소크 — 2500발을 돌려 어느 동전이 한 번이라도 걸리는가.
...
#      안 걸리므로 측정기 표로만 판단한다.
#    · 측정기는 동전을 **한 장씩** 재고 판은 무보드 확장 기본판이다. 다른
#      동전나 보드 확장이 있어야 서는 조건은 두 자 모두에서 0 으로 나온다.
...
# 36만 표본짜리 측정기 표. 실전 소크에서 0회인 동전이 여기서도 0.00 이면
