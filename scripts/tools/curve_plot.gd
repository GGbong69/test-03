...
func _bar(v: float, lo: float, hi: float, w: int) -> String:
	# 로그 자로 잰다. 총 배율이 328배라 선형 막대면 앞 다섯 앤티가 다 0칸이다.
...
func _initialize() -> void:
	...
	print("무장 없는 한 라운드 기댓값 %.0f점 (%d발 x 14.4)\n"
			% [unarmed, GameData.tune_i("darts_base")])

	var lo := GameData.ante_base(1)
	var hi := GameData.ante_base(n) * 2.0
	print("앤티   기본     작은     큰     보스   배율   로그 자")
	var prev := 0.0
	for a in range(1, n + 1):
		var b := GameData.ante_base(a)
		# 판 번호로 되돌려 실제 목표를 읽는다 — 판돈·팩 배수까지 탄 값이다.
		var r0 := (a - 1) * GameData.blinds_per_ante() + 1
		var row := ""
	...
	for a in range(1, mini(n, BAL.size()) + 1):
		print("A%d %9.1f %10.1f"
				% [a, GameData.ante_base(a) / GameData.ante_base(1), BAL[a - 1] / BAL[0]])

	print("\n총 배율 %.0f배 · 첫 판 목표가 무장 없는 기댓값의 %.0f%%"
			% [GameData.ante_base(n) / GameData.ante_base(1),
