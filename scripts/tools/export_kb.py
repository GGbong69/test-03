...
def pack_desc(p: dict) -> str:
    """data.gd 의 pack_desc — 다트통 설명은 총량과 증감을 둘 다 꽂는다.
...
RULES = {
    ...
    "core_loop": {
        ...
        "sequence": ["제목", "새 런(다트통·리그 선택)", "판 선택(던지기/건너뛰기)",
                     "[보스 앞] 제약 선택", "조준 → 착탄 → 정산 반복", "판 정산(골드)",
                     "상점", "다음 판"],
    },
    "scoring": {
        "formula": "한 발의 점수 = 기본 점수 × 배수 (score_mode 'std'). 'bal' 은 ((기본 점수+배수)/2)² 이다.",
        "chip_source": "꽂힌 칸의 값(1~20 · 불 25 · 이너 불 50 · 보드 아웃 0).",
        "mult_source": "꽂힌 영역의 배수(싱글 1 · 더블 2 · 트리플 3 · 불 1 · 빗나감은 큐가 1을 넣는다).",
        "queue": ["기본 점수(칸 값)", "배수(영역)", "[관통 다트] 양옆 칸 평균의 절반",
                  "발동한 동전를 동전 슬롯 순서대로", "합산 — 기본 점수 × 배수를 총점에 더한다"],
        "order_note": "동전는 동전 슬롯에 꽂힌 순서대로 정산된다. ×배수가 뒤에 오면 앞의 +배수를 "
                      "전부 곱하므로 순서가 곧 빌드다.",
        "kill_axes": "칸을 죽이는 축은 번호 → 색 → 홀짝 순으로 좁아진다. 링을 죽이는 축은 "
                     "값은 두고 배수만 1로 내린다.",
        "dart_then_track": "다트 보정(fix1 · mult · pierce)이 먼저, 트랙이 그 뒤다. "
    ...
    "board": {
        ...
        "order": "bi ≤ bo < t2i < t2o < ti < to < din < dout — hit_info 의 if 사슬이 "
                 "전제하는 순서 그 자체다. 보드 확장가 이 순서를 깨면 테이블에 안 뜬다.",
        "board_val": "판값 = 균등 조준 다트당 기대 점수. 기본 판 %.4f. 보드 확장가 이 값을 "
                     "기본의 %s배 넘게 올리면 테이블에 안 뜬다."
    ...
    "economy": {
        ...
        "start": "시작 자금 %d골드. 다트통이 이 값을 덮는다." % TUNE["start_gold"],
        ...
        "slots": "동전 슬롯 %d칸 · 사탕 칸 %d칸. 다트통이 덮는다."
                 % (TUNE["max_items"], TUNE["cons_slots"]),
    },
    "shop": {
        "when": "판 N 을 클리어한 뒤 N+1 을 위해 열린다. 마지막 판 뒤에는 안 연다.",
        "stock": "테이블 자리는 정확히 4개다 (동전 %d · 보드 확장 %d · 다트 %d)."
                 % (ANTES[0]["shop_items"], ANTES[0]["shop_mods"], ANTES[0]["shop_darts"]),
        "weighted": "동전는 등급 가중치로 뽑는다. 이미 가진 것과 아직 안 풀린 것은 "
                    "애초에 후보에서 빠진다.",
        "mods": "판이 더는 못 받는 보드 확장(기하 순서 위반 · 판값 상한 초과)와 이미 산 보드 확장는 "
                "후보에서 빠진다. 무효 구매가 존재할 수 없다.",
        "voucher": "장갑는 테이블가 아니라 왼쪽 벽에 걸린다. 앤티당 하나, 리롤으로 안 씻긴다.",
        "physics": "매물은 상인가 펠트 위로 던져 굴린다. 리롤하면 쓸어 담고 다시 던진다. "
    ...
    "meta": {
        ...
        "unlock_packs": "기본 다트통은 완주로 표 순서대로 열린다. 히든 다트통은 통계 조건으로 열린다.",
        "unlock_stakes": "리그는 앞 단을 완주하면 다음 단이 열린다.",
    },
    "undecided": {
        "processing": "스티커 %d종 — 전부 제안 상태, enabled=0. 대상 선택 흐름 미구현."
                      % len(PROCESSING),
        "consumables": "사탕은 전부 '가안'. 가격이 미정이라 임시가 %d골드를 쓴다."
                       % TUNE["cons_price_tmp"],
        "area_upgrades": "트랙은 표와 코드가 다 서 있고 수치도 들어갔다. "
                         "다만 사탕 테이블 구성(shop_cons)이 임시 규칙이다.",
        "hidden_pack": "히든 다트통 p_x1 은 자리만 세워 둔 것이다 — 지금은 기본과 똑같이 돈다.",
...
def build_md() -> str:
    ...
    w("1. 기획서 초안을 쓰게 하려면 이 `.md` 를 통째로 올린다. 문장과 표가 같이 있어 "
      "맥락이 안 끊긴다.")
    w("2. 수치 계산·밸런스 시뮬레이션·표 재스티커을 시키려면 `hightone_kb.json` 을 올린다. "
    ...
    w("**한 줄 요약** — 던지는 손(조준)과 읽는 머리(빌드)가 **곱해지는** 다트 로그라이트. "
      "꽂힌 자리가 앞의 수를, 들고 있는 동전가 뒤의 수를 정하고 둘은 더해지지 않고 곱해진다.\n")
    w("**규모**\n")
    c = build_json()["derived"]["counts"]
    w(md_table(["갈래", "수"], [
        ["동전(아이템)", "%d장 (활성 %d · 대기 %d)" % (c["items_total"], c["items_enabled"], c["items_disabled"])],
    ...
    w("- 목표를 못 넘기면 런이 끝난다. 단, `save` 종류 동전가 있으면 1회 무를 수 있다.")
    w("\n---\n")
    ...
    w("### 3-1. 공식\n")
    w("```")
    w("  한 발의 점수 = 기본 점수(chip) × 배수(mult)")
    w("  라운드 총점 = Σ 한 발의 점수")
    w("  클리어 조건 = 총점 ≥ 목표")
    w("```\n")
    w("- **기본 점수**: " + sc["chip_source"])
    w("- **배수**: " + sc["mult_source"])
    w("- 계산 방식은 다트통이 쥔다. `std` = 기본 점수 × 배수 · `bal` = ((기본 점수+배수)/2)². "
    ...
    w(md_table(["열쇠", "값", "뜻"], [
        ...
        ["ti", bb["ti"], "트리플 띠 안쪽"],
        ["to", bb["to"], "트리플 띠 바깥"],
        ["t2i", bb["t2i"], "둘째 트리플 띠 안쪽 (0 = 없음 · 보드 확장 '아랫목'이 켠다)"],
    ...
    w("조건이 맞은 동전만 그 다트에서 발동한다. 동전 슬롯에 꽂힌 **순서대로** 정산되므로 순서가 곧 빌드다.\n")
    ...
    # 갈래는 셋이고 겹치지 않는다 — 조준 동전 → 스펙 유래(j###) → 나머지가 기본 세트.
    ...
    w("점수도 배수도 안 준다. **드는 순간 이 런의 조준 방식이 바뀐다.** "
      "다트통이 아니라 동전라서 사고 팔고 봉인되는 규칙 아래 놓인다.\n")
    ...
    w("판은 상태가 아니라 산 보드 확장 목록의 함수다 — 살 때마다 기본 판에서 표 순서대로 다시 굽는다. "
      "그래서 \"먼저 산 보드 확장가 나중 보드 확장를 흔드는가\" 라는 물음이 아예 성립하지 않는다.\n")
    w("- 기하 순서를 깨거나 판값을 기본의 %s배 넘게 올리는 보드 확장는 **테이블에 안 뜬다**." % TUNE["val_max_mul"])
    ...
    w("- `mult` — 배수 보정. 하한 1 을 통과한 뒤 트랙 강화가 그 위에 얹힌다.")
    w("- `fix1` — 배수를 1로 고정한다. 트리플·더블·링 조건 동전가 전부 죽는다.")
    w("- `pierce_side` — 양옆 칸 값의 이 비율만큼을 기본 점수으로 더 받는다.")
    ...
    w("테이블 구성도 임시 규칙이다 — `shop_cons`(현재 %d)가 켜져 있으면 동전 자리 하나를 "
      "사탕으로 바꾼다.\n" % TUNE["shop_cons"])
    ...
    w("\n### 9-1. 트랙 (발라트로의 행성 카드 자리)\n")
    w("사탕이 트랙 레벨을 올리고, 1..lv 레벨 행의 수치를 합친 것이 착탄에 얹힌다.")
    ...
    # 11 tags
    w("## 11. 뱃지 (건너뛰기 보상)\n")
    w("작은 판과 큰 판은 건너뛸 수 있다. 건너뛰면 점수도 골드도 없고 대신 뱃지를 받는다 — "
    ...
    w("- 테이블가 아니라 **왼쪽 벽**에 걸린다. 리롤해도 안 씻긴다.")
    ...
    w(md_table(["id", "이름", "미는 값", "v", "연산", "가격", "선행", "최소 앤티", "가중치", "효과", "비고"],
               [[v["id"], v["name"], v["key"], v["v"], v["op"], v["cost"], g(v["prereq"]),
                ...
                for v in VOUCHERS]))
    w("\n**장갑가 밀 수 있는 축과 범위** (`VOUCHER_KEYS` — 목록이 곧 계약이다)\n")
    w(md_table(["열쇠", "바닥", "천장"],
    ...
    # 13 packs
    w("## 13. 다트통\n")
    w("발라트로의 덱 자리다. 런의 **시작 조건**을 쥔다. 기본 다트통은 완주하면 표 순서대로 다음 것이 열린다.\n")
    w("다트통은 효과를 직접 들고 있지 않고 **물건으로** 준다(`grant_*`). "
      "그래야 그 물건이 사고 팔고 봉인되는 규칙 아래 놓인다.\n")
    w(md_table(["id", "이름", "갈래", "선행", "다트±", "골드±", "동전 칸", "사탕 칸",
    ...
    w("리그는 곡선을 **고르는** 것이지 곱하는 것이 아니다 — `curve` 열이 `antes.csv` 의 "
      "어느 열을 볼지 가리킨다.\n")
    w(md_table(["id", "이름", "곡선", "작은판 보상", "봉인", "다트±", "테이블가", "이자상한+",
                "삭음", "유지비", "선행", "비고"],
    ...
    w("")
    w("- **봉인** — 매 판 동전 하나가 무작위로 봉인된 채 시작한다(제약 '둔화'의 상시판).")
    w("- **삭음** — 산 동전가 N판 뒤에 부서진다.")
    ...
    w(md_table(["앤티", "기본", "curve_a", "curve_b", "테이블(동전/보드 확장/다트)", "다트", "비고"],
    ...
    w(md_table(["id", "이름", "목표 배수", "보상 골드", "건너뛰기", "보스", "비고"],
               [[b["id"], b["name"], b["mult"], b["reward"],
                ...
                for b in BLINDS]))
    w("\n**펼친 진행표 — 흰 리그 기준 (%d판)**\n" % ROUNDS_N)
    w(md_table(["판", "앤티", "종류", "목표", "다트", "보상", "건너뛰기", "다음 상점(동전/보드 확장/다트)"],
    ...
    w(md_table(["입력", "하는 일"], [
        ...
        ["마우스 드래그", "상점 — 매물을 계산대로 민다 / 동전 슬롯 — 동전 순서 바꾸기·판매"],
    ...
    w("- 다트판 중심 (%g, %g), 반지름 %s px." % (CODE["screen"]["BOARD_CENTER"][0],
                                                ...
                                                TUNE["board_r"]))
    w("- 상시 HUD — 상단바(목표·총점) · 자금판 · 동전 슬롯 · 사탕 칸 · 남은 다트 부채.")
    w("- 상점은 오버헤드 정사영(앙각 52°)으로 그린 펠트 테이블이다. 상인가 매물을 던지고 쓸어 담는다.")
    ...
    w(md_table(["이름", "hex", "쓰임"],
               [[k, v, {"C_BG": "배경", "C_LIGHT": "밝은 칸", "C_DARK": "어두운 칸",
                        ...
                        "C_PANEL": "패널", "C_CHIP": "기본 점수(점수)", "C_MULT": "배수",
                        "C_GOLD": "골드", "C_FELT": "동전 슬롯 펠트", "C_TABLE": "상점 펠트",
                        "C_WOOD": "나무", "C_DIECUT": "동전 다이컷 테두리",
                        "C_LINER": "이형지"}.get(k, "")]
    ...
    w(md_table(["게임 안 말", "코드 이름", "뜻"], [
        ["동전", "item / owned", "발라트로의 조커. 동전 슬롯에 %d칸" % TUNE["max_items"]],
        ["보드 확장", "mod / mods_own", "판의 기하를 바꾸는 영구 구매"],
    ...
    for k, v in RULES["undecided"].items():
        w("- **%s** — %s" % (k, v))
    w("")
    w("**엔진이 아직 못 하는 것** (대기 동전 %d장의 비고에서 뽑았다)\n" % len(OFF))
    reasons = {}
    for i in OFF:
    ...
    w("**열이 뜻하는 것**\n")
    w(md_table(["열", "뜻"], [
        ["fire_pct", "발동률(%) — 이 동전가 한 발에서 조건을 만족할 확률"],
