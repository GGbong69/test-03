...
RULES = {
    ...
    "scoring": {
        ...
        "queue": ["기본 점수(칸 값)", "배수(영역)", "[관통 다트] 양옆 칸 평균의 절반",
                  "발동한 동전을 동전 슬롯 순서대로", "합산 — 기본 점수 × 배수를 총점에 더한다"],
        "order_note": "동전은 동전 슬롯에 꽂힌 순서대로 정산된다. ×배수가 뒤에 오면 앞의 +배수를 "
                      "전부 곱하므로 순서가 곧 빌드다.",
    ...
    "board": {
        ...
        "order": "bi ≤ bo < t2i < t2o < ti < to < din < dout — hit_info 의 if 사슬이 "
                 "전제하는 순서 그 자체다. 보드 확장이 이 순서를 깨면 테이블에 안 뜬다.",
        "board_val": "판값 = 균등 조준 다트당 기대 점수. 기본 판 %.4f. 보드 확장이 이 값을 "
    ...
    "economy": {
        "income": ["판 클리어 보상 (작은 %d · 큰 %d · 보스 %d)"
                   ...
                   "동전 판매",
                   "건너뛴 판의 뱃지"],
        "outgo": ["테이블 구매(동전 · 보드 확장 · 다트 · 사탕)", "유료 리롤", "사진 구매",
                  "리그 유지비 — 판마다 %d골드 (%s)"
    ...
    "shop": {
        ...
        "weighted": "동전은 등급 가중치로 뽑는다. 이미 가진 것과 아직 안 풀린 것은 "
                    "애초에 후보에서 빠진다.",
...
def build_md() -> str:
    ...
    w("1. 기획서 초안을 쓰게 하려면 이 `.md` 를 통째로 올린다. 문장과 표가 같이 있어 "
      "맥락이 안 끊긴다.")
    w("2. 수치 계산·밸런스 시뮬레이션·표 재스티커를 시키려면 `hightone_kb.json` 을 올린다. "
    ...
    w("**한 줄 요약** — 던지는 손(조준)과 읽는 머리(빌드)가 **곱해지는** 다트 로그라이트. "
      "꽂힌 자리가 앞의 수를, 들고 있는 동전이 뒤의 수를 정하고 둘은 더해지지 않고 곱해진다.\n")
    w("**규모**\n")
    ...
    w("- 목표를 못 넘기면 런이 끝난다. 단, `save` 종류 동전이 있으면 1회 무를 수 있다.")
    w("\n---\n")
    ...
    w("판은 상태가 아니라 산 보드 확장 목록의 함수다 — 살 때마다 기본 판에서 표 순서대로 다시 굽는다. "
      "그래서 \"먼저 산 보드 확장이 나중 보드 확장을 흔드는가\" 라는 물음이 아예 성립하지 않는다.\n")
    w("- 기하 순서를 깨거나 판값을 기본의 %s배 넘게 올리는 보드 확장은 **테이블에 안 뜬다**." % TUNE["val_max_mul"])
    ...
    w("- `fix1` — 배수를 1로 고정한다. 트리플·더블·링 조건 동전이 전부 죽는다.")
    w("- `pierce_side` — 양옆 칸 값의 이 비율만큼을 기본 점수로 더 받는다.")
    ...
    w("리그는 곡선을 **고르는** 것이지 곱하는 것이 아니다 — `curve` 열이 `antes.csv` 의 "
      "어느 열을 볼지 가리킨다.\n")
    w(md_table(["id", "이름", "곡선", "작은판 보상", "봉인", "다트±", "테이블이", "이자상한+",
                "삭음", "유지비", "선행", "비고"],
    ...
    w("- 상점은 오버헤드 정사영(앙각 52°)으로 그린 펠트 테이블이다. 상인이 매물을 던지고 쓸어 담는다.")
    ...
    w("**열이 뜻하는 것**\n")
    w(md_table(["열", "뜻"], [
        ["fire_pct", "발동률(%) — 이 동전이 한 발에서 조건을 만족할 확률"],
