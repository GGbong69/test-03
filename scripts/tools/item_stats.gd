...
# ══════════════════════════════════════════════════════════
#  기본 점수 기여 측정기 — data/_stats.csv 를 덮어쓴다
#
#  실행:  godot --path . --headless --script scripts/tools/item_stats.gd
#
#  재는 것
#    발동률   그 기본 점수의 조건이 다트 한 발에서 걸릴 확률
#    기여     그 기본 점수 한 장이 다트당 더해 주는 점수의 기대값
#
#  기여를 재는 이유는 가격과 나란히 놓기 위해서다. 조건이 넓고 값이 작은 기본 점수과
#  조건이 좁고 값이 큰 기본 점수을 같은 자로 재야 비싼 값을 하는지 알 수 있다.
#
#  수량은 게임과 같은 GameData.item_amt 로 계산한다. 측정기가 제 셈을 하면
#  게임과 갈라진 날 표 전체가 조용히 거짓말이 된다.
#
#  전제 넷 — 이 넷이 바뀌면 표의 모든 수가 통째로 무의미해진다
#    ① 보드 확장 없는 기본 판(board=base). 보드 확장 여덟은 링 폭과 칸 값을 바꾸므로
#       "좁은 판" 제약 하나만 걸려도 링 조건 넉 장의 발동률이 반토막 난다.
#    ② 기본 점수은 한 장만 들었다고 본다(build=solo). 다른 기본 점수과의 곱은 안 본다.
#       보유량 배율은 이 전제를 그대로 따른다 — 아이템 1, 빈 슬롯 나머지,
#       동전 슬롯의 남 판매가 0. 골드·무거운 다트 배율은 경제를 모사하지 않으므로
...
func _initialize() -> void:
	...
	print("다트 %d발 · 빗나감 %.2f%% · 무실책 판 %.2f%% · 무보드 확장 판 다트당 기본 %.2f점"
...
# 한 발에서 이 효과가 늘리는 점수. solo 전제라 다른 기본 점수의 배수는 없다.
