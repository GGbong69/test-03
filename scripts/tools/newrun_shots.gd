...
# 자리 겹침 자체는 league_probe 가 수로 재고, 여기는 눈으로 본다.
#
...
func _run() -> void:
	...
	for k in st.size():
		GameData.league = String(st[k].get("id", ""))
		await _wait(2)
		var name := "newrun_%d_%s.png" % [k, GameData.league]
		root.get_texture().get_image().save_png("res://shots/" + name)
		print("저장: %-22s (%d줄)" % [name, g._league_lines().size()])
	quit()
