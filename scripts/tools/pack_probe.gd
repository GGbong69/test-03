...
func _initialize() -> void:
	...
	# ⑦ 기본 팩은 완주로 열린다 — 표에 다음 기본이 있을 때만 뜻이 있다
	var bases := GameData.packs_of("base")
	Save.wipe()
	GameData.pack = String(bases[0].get("id", ""))
	g._pack_unlock_next()
	if bases.size() < 2:
		_say(true, "기본 팩 사다리", "다음 기본 팩이 아직 없다 (표 %d행)" % bases.size())
	else:
		_say(Save.unlocked("pack:" + String(bases[1].get("id", ""))),
				"완주가 다음 기본 팩을 연다", String(bases[1].get("name", "")))
