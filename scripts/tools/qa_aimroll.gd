...
func _run() -> void:
	...
	#
	#  ⚠ **바닥이 3 이었다. 그 자가 제 손으로 빨개진다**(2026-09-24).
	#  레전더리는 2500장에 다섯 장쯤이라 두 수가 다 푸아송(λ≈5)이다.
	#  R1 이 0·1 로 굴러 떨어지면 바닥이 3 으로 눌리는데 R8 이 3 을 넘을
	#  확률은 그 자리에서 73% 다 — 한 번 재 보니 R1 0번 · R8 8번으로 울었고
	#  같은 코드를 다시 돌리니 R1 5번 · R8 5번으로 통과했다. 아무것도
	#  안 고쳤는데 **열여덟 번에 한 번쯤 빨개지는** 자였다.
	#  바닥을 12 로 올린다: 푸아송(5)에서 12 를 넘을 확률이 0.2% 라 잡음에
	#  안 울고, 머리말이 말하는 **6배 갈림**(λ≈30)은 그대로 잡는다.
	#  위쪽 R1×3 은 그대로 둔다 — R1 이 제대로 굴렀을 때의 계약은 안 건드린다.
	_ok("팩의 레전더리가 라운드를 안 탄다", int(pk_leg[1]) <= maxi(int(pk_leg[0]) * 3, 12),
