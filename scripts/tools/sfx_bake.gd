...
# 펠트 깐 나무 카운터 — 상점의 몸통. 판·막대 비율과 같은 줄을 쓴다.
const WOOD := [1.0, 2.76, 5.40, 8.93]
...
func _b_shop(nm: String) -> PackedFloat32Array:
	match nm:
		# ══ 상점 ═════════════════════════════════════════
		#  가구 **하나**로 낸다. 펠트 깐 나무 카운터(1 : 2.76 : 5.40 : 8.93)가
		#  몸통이고, 그 위에 얹히는 얇은 금속 원판(1 : 2.08 : 3.41 : 5.06)이
		#  알맹이고, 둘 사이를 같은 펠트 잡음이 잇는다. 상한은 6kHz —
		#  9.7kHz 유리 딸깍은 이 가구에 없다.
		#
		...
		"buy":
			...
			_addat(x, t2, 70.0)
			# 나무 턱 — 가족을 한 가구로 묶는 접착층. 무게는 한 번만 준다
			var jw := _pt(60.0, 196.0, _rat(WOOD, [1.00, 0.34, 0.14],
					[0.035, 0.014, 0.008]), 0.8)
			_lay(x, jw, 0.28)
			_lay(x, jw.duplicate(), 0.28, 70.0)
		...
		"sell":
			...
			_lay(x, _pt(40.0, 196.0, _rat(WOOD, [1.00, 0.32],
					[0.022, 0.010]), 0.8), 0.22, 45.0)
			...
			return _fade(x, 12.0)
		"deny":
			# 물린 노크 둘. **원판 비율을 한 개도 안 쓴다** — 오간 것이 없으니
			# 금속 울림이 남으면 안 된다
			var x := _blank(_n(150.0))
			_addat(x, _pt(120.0, 200.0, _rat(WOOD, [1.00, 0.30, 0.09],
					[0.030, 0.012, 0.006]), 0.5))
			_addat(x, _pt(90.0, 150.0, _rat(WOOD, [0.95, 0.26, 0.07],
					[0.034, 0.013, 0.006]), 0.5), 60.0)
		...
		"reroll":
			...
			# 가족의 나무 턱. 220 은 shop_open 의 220 과 같은 자리다
			_lay(x, _pt(120.0, 220.0, _rat(WOOD, [1.00, 0.37, 0.13],
					[0.055, 0.020, 0.010]), 3.0), 0.30)
			...
			return _fade(x, 10.0)
		"fixture_buy":
			# 알맹이가 원판이 아니라 **판재**다 — 8.93 부분음이 이 이름에만
			# 있다. 셋을 392-523-659 로 올려 박고 상자가 쿵 한다
			...
			_addat(x, _pt(200.0, 392.0, _rat(WOOD, [1.00, 0.40, 0.16, 0.06],
					[0.055, 0.024, 0.013, 0.007]), 0.8))
			_addat(x, _pt(180.0, 523.0, _rat(WOOD, [1.00, 0.36, 0.14, 0.05],
					[0.060, 0.026, 0.014, 0.007]), 0.8), 70.0)
			_addat(x, _pt(160.0, 659.0, _rat(WOOD, [0.62, 0.32, 0.12, 0.04],
		...
		"cons_use":
			...
			return _fade(x, 10.0)
		"sweep_sink":
			# 밀대가 알을 구멍으로 떨군다. 금속 부분음 0 개.
			# 한두 프레임에 대여섯이 같이 나므로 감쇠를 짧게 맨다
			var x := _blank(_n(55.0))
			_addat(x, _pt(50.0, 196.0, _rat(WOOD, [1.00, 0.28, 0.08],
					[0.010, 0.007, 0.004]), 0.3))
...
# 나무 카운터 — 메뉴의 배음 몸통 1 : 2.00 : 3.01 : 4.00.
const TUB := [1.0, 2.00, 3.01, 4.00]
...
func _b_menu(nm: String) -> PackedFloat32Array:
	match nm:
		...
		"newrun_open":
			...
			_addat(x, _pt(90.0, 440.0, _rat(TUB, [0.55, 0.75, 0.45, 0.20],
					[0.030, 0.022, 0.014, 0.008]), 0.8))
			_lay(x, _chip(30.0, 523.0, 0.0040, 13, 0.40), 0.30)
			_lay(x, _clk(20.0, 3.0, 900.0, 900.0, 1.0, 0.0025, 11, 0.3), 0.30)
			_addat(x, _pt(60.0, 440.0, _rat(TUB, [0.17, 0.23, 0.14, 0.06],
					[0.021, 0.015, 0.010, 0.006]), 0.8), 22.0)
		...
		"menu_back":
			...
			return _fade(x, 4.0)
		"back":
			# 테이블을 한 번 노크한다. **금속도 종도 안 넣는다** —
			# 상한 3000Hz 로 이 가족의 바닥을 만든다
			var x := _pt(55.0, 330.0, _rat(TUB, [1.00, 0.30, 0.10],
					[0.018, 0.009, 0.005]), 0.6)
