...
func _b_throw(nm: String) -> PackedFloat32Array:
	match nm:
		...
		"dart_pick":
			...
			#  ⚠ **던지기 가족만 천장(_lp)이 한 개도 없었다** — 다섯 함수를
			#  다 읽었고 어느 것도 안 불렀다. 착탄 가족은 6500, 판·런은
			#  6000~4600, 정산은 2600~1200 으로 전부 천장이 있는데 여기만
			#  맨머리였다. 새는 층은 이 이탈 마찰 한 줄이다.
			#  ⚠ **심사가 든 수는 못 맞췄다.** 「dart_pick 이 6kHz 위 9.01%
			#  로 66개 중 1위」라 했는데 이 저장소의 자(probe_sfx 의 _above)
			#  로 재니 1.0% 로 넷째였다 — 그 수로는 「샌다」가 아니다.
			#  그래도 천장은 단다: 고치는 이유가 **누출이 아니라 한 집안**
			#  이다. 이 소리는 저 뒤에 오는 착탄 가족(6500)과 한 동작인데
			#  혼자 지붕이 없어, 나중에 이 자리에 밝은 층이 한 줄 더 들어와도
			#  아무것도 막지 않는다. 크기는 안 바뀐다(_norm_rms 가 에너지로
			#  맞추므로 피크만 조금 내려간다). 2026-09-24
			_lp(x, 6000.0)
			return _fade(x, 8.0)
		"aim_lock_first":
		...
		"dart_fly":
			...
			#  착탄 가족과 **같은 칸**에 세운다 — 손을 떠난 다트와 그것이
			#  박히는 소리가 한 지붕 밑에 있어야 한 물건의 소리가 된다.
			#  2026-09-24
			_lp(x, 6500.0)
			return _fade(x, 12.0)
...
func _b_hit(nm: String) -> PackedFloat32Array:
	match nm:
		...
		"board_thud":
			...
			return _fade(x, 10.0)
		"board_crack":
			# 금이 한 단 나아간다 — 나무 결이 지직 하고 한 뼘 갈라진다.
			# **board_break 의 ① 삐걱 한 줄을 여리게 뽑은 것**이다: 같은
			# 판이 내는 소리이므로 같은 기구에서 나와야 하고, 그 뒤에 오는
			# 한 방과 한 집안으로 들려야 한다. 콰득이 아니라 삐걱이라
			# 중심을 700/350 그대로 두고 길이만 36ms 로 자른다.
			# 바닥에 결 찢김을 아주 여리게 한 겹 — 그것이 「번진다」다.
			# 천장은 board_break 와 같은 4600 이다. 쇠줄(2870)은 **안 넣는다**:
			# 철사가 끊기는 것은 판이 실제로 갈라지는 그 한 번뿐이라,
			# 여기 섞으면 발화의 신호가 닳는다. 2026-09-24
			var x := _blank(_n(36.0))
			_lay(x, _clk(24.0, 2.0, 700.0, 350.0, 0.8, 0.008, 7321, 0.25), 1.00)
			_lay(x, _nz(30.0, 1400.0, 500.0, 0.9, 7322, 0.010, 3.0), 0.22, 3.0)
			_addat(x, _pt(34.0, 185.0, [[1.0, 0.34, 0.009],
					[2.31, 0.12, 0.005]], 0.6), 1.0)
			_drive(x, 1.2)
			_lp(x, 4600.0)
			return _fade(x, 6.0)
		"board_break":
			# 다트판이 깨진다 — 목표를 넘긴 판이 금을 따라 갈라져 뜬다.
			# 판 **위** 소리라 유리가 아니라 나무·철선이다. egg_crack 과
			# 같은 집이지만 **다른 사건**이어야 한다: 두 판이 같은 소리로
			# 깨지면 둘이 같은 물건이 된다. 제목 판보다 크고 무르므로
			# 딸깍 중심을 내리고(1500/900 → 700/520) 천장도 내린다.
			var x := _blank(_n(300.0))
			# ① 삐걱 — 콰득이 아니다. egg_crack 의 딱 넷에서 하나를 빼고
			#    중심을 내린다. 3kHz 위로 올리면 나무가 아니라 유리 딸깍이
			#    된다(egg_crack 이 스펙트럼 무게중심으로 이미 잰 값이다).
			_lay(x, _clk(50.0, 4.5, 700.0, 350.0, 0.8, 0.014, 7301, 0.25), 1.00, 0.0)
			_lay(x, _clk(44.0, 4.0, 520.0, 260.0, 0.8, 0.012, 7302, 0.25), 0.80, 38.0)
			_lay(x, _clk(36.0, 3.0, 980.0, 490.0, 0.9, 0.010, 7303, 0.20), 0.55, 11.0)
			# ② 나무 속 — 비배음. egg_crack 의 185Hz 를 140 → 180ms 로 눕힌다
			_addat(x, _pt(180.0, 185.0, [[1.0, 1.10, 0.034], [2.31, 0.50, 0.017],
					[3.92, 0.22, 0.008]], 0.6), 2.0)
			# ③ **철사** — 이 한 겹이 「다트판이 깨졌다」를 「제목 판이
			#    쪼개졌다」와 가른다. 1 : 1.509 는 배음이 아니라 쇠고
			#    _hit_c 의 그 비다. 이 표 어디에도 **끊기는** 쇠줄이
			#    없었다 — 더블·트리플의 딸깍뿐이다.
			_addat(x, _pt(60.0, 2870.0, [[1.0, 0.30, 0.010],
					[1.509, 0.18, 0.005]], 0.35), 26.0)
			# ④ 결 찢김 — egg_crack 의 지직 바닥(160ms·0.20)을 길고 여리게
			_lay(x, _nz(200.0, 1400.0, 500.0, 0.9, 7310, 0.060, 4.5), 0.16, 48.0)
			# ⑤ 판이 마지막으로 운다 — 착탄 여섯이 다 쓰는 98Hz 판 모드.
			#    tau 를 이 표에서 제일 길게 둔다: 깨진 뒤 남는 것은
			#    몸통의 울림이다.
			_addat(x, _pt(320.0, 98.0, [[1.0, 0.60, 0.080],
					[2.755, 0.20, 0.028]], 2.2))
			_drive(x, 1.6)
			# egg_crack 의 5000 **밑**. 반려된 9.7kHz 유리 대역을 한 겹 더
			# 비운다 — 이쪽이 더 크고 더 무른 판이다.
			_lp(x, 4600.0)
			return _fade(x, 22.0)
		"egg_crack":
...
func _b_leg(nm: String) -> PackedFloat32Array:
	match nm:
		...
		"leg_open":
			...
			#  ⚠ **천장이 아예 없는데 미는 자리다.** _leg_open_f() 가 R8 에서
			#  x1.4983 으로 민다(page 와 같은 비 — 한 경계가 한 목소리로
			#  말한다). 천장이 없으니 미끄러짐의 꼭대기 5200 이 그대로 밀려
			#  6kHz 위 몫이 5.6% 로 표에서 가장 밝았다(probe_sfx 「지붕」).
			#  종이가 미끄러지는 소리가 R8 에서 쇳소리로 들리는 자리다.
			#  미끄러짐의 꼭대기를 3000 으로 내리고 천장을 3600 에 세운다 —
			#  재어 가며 앉힌 수다(5.6% → 1.7%). 미는 배율은 안 건드린다.
			#  2026-09-24
			var x := _blank(_n(190.0))
			_lay(x, _slide(95.0, 3000.0, 1500.0, 1.1, 1105, 4.0, 12.0, 0.35), 0.55)
...
func _b_menu(nm: String) -> PackedFloat32Array:
	match nm:
		...
		"run_start":
			...
			return _fade(x, 8.0)
		"endless_go":
			# **레버가 한 음 높이 걸린다.** run_start 와 같은 몸이되 178 →
			# 200 · 247 → 277 · 98 → 110 으로 한 단(9/8) 올리고 디텐트도
			# 196 → 220 이다 — 「같은 레버, 한 칸 더」가 무한의 뜻이라
			# 새 어휘를 안 들인다. 길이는 표의 d(0.08). 2026-09-20
			var x := _blank(_n(82.0))
			_addat(x, _one(76.0, 200.0, 180.0, 0.017, 1.0))
			_addat(x, _pt(66.0, 277.0, [[1.0, 0.55, 0.014]], 1.0))
			_addat(x, _pt(76.0, 110.0, [[1.0, 0.40, 0.020]], 1.0))
			_drive(x, 1.4)
			_lay(x, _clk(16.0, 4.0, 3200.0, 2600.0, 1.8, 0.0025, 43, 0.3), 0.45)
			_lay(x, _detent(34.0, 220.0, 44, 0.60, 0.24), 0.45, 30.0)
			return _fade(x, 8.0)
		"menu_pick":
		...
		"page":
			...
			# 결 두 겹뿐이다. 중심을 내리고 상한을 3단으로 묶었다: 전에 이
			# 자리가 5kHz 위에 에너지의 절반을 두고 있었다
			#  ⚠ **그 고침을 사다리가 통째로 되돌리고 있었다.** 이 소리는
			#  _turn_f() 가 R8 에서 x1.4983 으로 미는 자리다(라운드마다 반음).
			#  파일이 있으면 _sfx 가 재생 속도를 그만큼 올리므로 **여기서 건
			#  천장도 같이 밀려** 5200 이 7792Hz 가 됐다 — 6kHz 위 몫이
			#  1.9% → 3.2%(probe_sfx 의 「지붕」으로 쟀다). _b_settle 은
			#  3.78배 사다리 때문에 부분음 상한을 따로 내렸는데 이 사다리에는
			#  그 셈을 한 번도 안 했다.
			#  **미는 배율은 안 건드린다** — 그것을 내리는 길은 라운드가
			#  오를수록 실리는 무게를 바꾸는 수치라 이번 일감이 아니다.
			#  굽는 천장만 내린다. 셈으로는 6000/1.4983 ≈ 4000 인데 3단
			#  필터가 모서리 바로 위를 다 못 깎아 거기서도 2.2% 가 남았다 —
			#  **재어 가며 3400 까지** 내려 1.9% 로 앉혔다. 결 두 겹의 중심도
			#  같이 2200/2400 으로 내려야 그 밑에서 결이 남는다. 2026-09-24
			var x := _blank(_n(34.0))
			var ga := _air(9.0, 1700.0, 2200.0, 1.4, 91)
			_atk(ga, 1.0)
			_dec(ga, 0.006)
			_lay(x, _fade(ga, 1.5), 1.00)
			var gb := _air(13.0, 2100.0, 2400.0, 1.4, 92)
			_atk(gb, 1.0)
			_dec(gb, 0.006)
