...
func _initialize() -> void:
	print("rounds.csv 에 줄을 더하면 (엔드리스) 기존 라운드가 어떻게 되나 — first=25 last=4000 bow=0.10")
	...
	for N in [8, 9, 10, 12]:
		...
		print(s)
	print("\n라운드 8 목표: N=8 → 4000 · N=9 → %.0f (%.0f%% 하락) · N=10 → %.0f · N=12 → %.0f"
	...
	print("\n── 생존 편향: 라운드별로 몇 런이 들어섰나 (보고된 통과율 역산) ──")
	var alive := 24.0
	...
	for a in range(1, 9):
		...
		print("  라운드 %d  들어선 런 %4.1f → 나온 런 %4.1f" % [a, enter, alive])
	print("  → 라운드 5·6 의 「사망 0」은 런 %d개 표본이다. 0/3 의 사망률 95%% 상한 = %.0f%%."
